...
class Result {
    ...
    public static List<Integer> circularArrayRotation(List<Integer> a, int k, List<Integer> queries) {
        List<Integer> arrayRotates = new ArrayList<Integer>();
        List<Integer> queryResults = new ArrayList<Integer>();

        k = k % a.size();

        arrayRotates = a.subList(a.size() - k, a.size());
        arrayRotates.addAll(a.subList(0, a.size() - k));

        for (int value : arrayRotates) {
            System.out.print(value + " ");
        }

        for (int index : queries) {
            queryResults.add(arrayRotates.get(index));
        }
